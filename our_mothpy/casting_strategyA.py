...
def create_trajectory_data(job_file_name = 'job.json',data_file_name ='data.json',
    ...
    navigator1 = mothpy_models.MothModular(sim_region, cd['x_start'], cd['y_start'],cd['nav_type'] , 2, cd['wait_type'], cd['speed'])
    ...
    def call_navigators(wait,cast,nav):
        for j in range(10):
            for i in range(20):
                ...
                new_navigator.y = 450 - i*3 #moths start at slightly varying positions
    ...
    dict_list = moth_simulation(cd['num_it'],
                                ...
                                cd['puff_spread_rate'],cd['speed'],cd['puff_mol_amount'],
                                1,
                                False)
