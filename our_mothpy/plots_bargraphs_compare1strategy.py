...
def present_graphs():
    ...
    for i in range(num_jobs):
        loop = str(i)
        data_list = get_data('sims/data'+loop+'.json',1)
        #print (data_list[0])
    ...
    if xlabel == 'puff_spread_rate':
        ...
        ax.set_xlabel(xlabel)
    ax.set_ylabel('Minimum Time to Female')

    
    set_charts(lib_mintime)
    plt.subplots_adjust(bottom=0.15)
    plt.xticks(index+bar_width*1.5, (str(values[0]), str(values[1]), str(values[2]),str(values[3])))
    ax.xaxis.label.set_fontsize(17)
    ax.yaxis.label.set_fontsize(17)
    plt.legend(legends)
    plt.savefig('Minimum Navigation Time vs '+ xlabel)
    plt.show()
    ...
    with open('sims/statistics.json', 'w') as outfile:
