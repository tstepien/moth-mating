...
%     c = odorFunTime_tues3pm(real(moths.z),imag(moths.z) , t, interval);
   ...
   c = odorFunTime_strategy(real(moths.z),imag(moths.z) , t, moths.interval,moths.Q,max( real(moths.z) ) );
    ...
    moths.z = moths.z + pars.dt.*moths.speed.*(exp(1i*moths.angles));% + (c>moths.quality).*exp(1i*pi));
    moths.z = (1-moths.success).*moths.z;


    %plotting
    if doplot==1 & mod(t,10)==0 
...
%             cont = fcontour(@(x,y) odorFunTime_strategy(x,y , t, moths.interval,moths.Q,max( real(moths.z) ) ),[-50 2000 -500 500],...
%                'LevelList',[min(moths.quality),max(moths.quality)],'MeshDensity',200);
            cont = fcontour(@(x,y) odorFunTime_strategy(x,y , t, moths.interval,moths.Q,1000000 ),[-50 1000 -500   500],...
               'LevelList',[10.^[-10:-6]],'MeshDensity',200);  
        end
