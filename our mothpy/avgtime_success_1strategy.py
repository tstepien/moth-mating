...
calculates average time as well as success percentage
"""
...
def search_efficiency(dict_list):
    ...
    for j in range(len(dict_list)): #iterate through navigators
        ...
        if diff_list[-1][-1]:
            for i in range(len(diff_list)): #iterate within list 
...
def minimum_time(dict_list):
    #calculate the minimum time of flight for successful flights
    times_list = []
...
def success_percentage(dict_list):
    winners = 0.
    for diff_dict in dict_list:
        diff_list = diff_dict["diff_list{0}".format(0)]
        #print diff_list[-1]
    ...
    success_percentage = winners / len(dict_list) *100      
...
def calc_stats(diff_dict):
    ...
    succ_perc = success_percentage(diff_dict)

    avg_time = average_time_relative(diff_dict)

    min_time = minimum_time(diff_dict)
    
    average_efficiency = search_efficiency(diff_dict)

    return [succ_perc,avg_time,min_time,average_efficiency]
...
def create_graphs(tot_stats,int_loop):
    ...
    #success percentage
    plt.figure()
    (succ_perc ,average_time_,average_efficiency) = zip(*tot_stats)
    ay = fig.add_subplot(111)
    index = np.arange(0, n_groups, 1)

    chart = plt.bar(index, succ_perc, bar_width, color = colors, edgecolor='black')
...
def multi_splice(list_dict,n):
    ...
    if length%n != 0:
        raise Exception('Number of navigators could not be divided into %0.1i' %n)
    spliced_lists = []
    lenn = int(length/n)
    for i in range(n):
        if i ==0 :
...
def check_for_duds():
    #checks for the number of navigators that have not moved for the entire simulation
...
if __name__ == "__main__":
    ...
    for i in range(1):
        loop = str(i)
        data_list = get_data('data'+loop+'.json',1)
        print(data_list[0])
